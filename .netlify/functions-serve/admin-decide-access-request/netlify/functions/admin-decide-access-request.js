...
// netlify/functions/admin-decide-access-request.ts
var admin_decide_access_request_exports = {};
__export(admin_decide_access_request_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(admin_decide_access_request_exports);
...
// netlify/functions/_invite-utils.ts
...
var generateInviteToken = () => (0, import_node_crypto.randomUUID)();
var generateInviteCode = () => {
  const code = (0, import_node_crypto.randomInt)(0, 1e6);
  return code.toString().padStart(6, "0");
};
var hashInviteCode = (token, code) => {
  return (0, import_node_crypto.createHash)("sha256").update(`${token}:${code}`).digest("hex");
};
var hashToken = (token) => {
...
var handler = async (event) => {
  ...
  const { data: requesterProfile } = await adminClient.from("profiles").select("role").eq("user_id", userData.user.id).single();
  if (!requesterProfile || requesterProfile.role !== "admin") {
    return { statusCode: 403, body: JSON.stringify({ error: "Admin access required" }) };
  }
  const { data: requestRow, error: requestError } = await adminClient.from("access_requests").select("*").eq("id", payload.request_id).single();
  ...
  const email = requestRow.email.toLowerCase();
  const role = requestRow.requested_role;
  ...
  const siteUrl = process.env.VITE_PUBLIC_SITE_URL || process.env.PUBLIC_SITE_URL || (event.headers?.origin ? event.headers.origin : null) || (event.headers?.host ? `https://${event.headers.host}` : "https://maximusscpi.com");
...
//# sourceMappingURL=admin-decide-access-request.js.map
