...
// netlify/functions/claim-invite.ts
var claim_invite_exports = {};
__export(claim_invite_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(claim_invite_exports);
...
var handler = async (event) => {
  ...
  let { data: invite, error: inviteError } = await adminClient.from("invitations").select("*").eq("token_hash", tokenHash).single();
  if (inviteError || !invite) {
    const fallback = await adminClient.from("invitations").select("*").eq("token", token).single();
  ...
  if (!safeEqual(expectedHash, invite.code_hash)) {
    return { statusCode: 400, body: JSON.stringify({ error: "Invalid code" }) };
  }
  const { data: userData, error: userError } = await adminClient.auth.admin.getUserByEmail(invite.email);
  ...
  await adminClient.from("invitations").update({ status: "USED", claimed_at: (/* @__PURE__ */ new Date()).toISOString(), claimed_by: userId }).eq("token_hash", invite.token_hash ?? tokenHash);
...
//# sourceMappingURL=claim-invite.js.map
