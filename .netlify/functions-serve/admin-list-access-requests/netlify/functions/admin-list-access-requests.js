...
// netlify/functions/admin-list-access-requests.ts
var admin_list_access_requests_exports = {};
__export(admin_list_access_requests_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(admin_list_access_requests_exports);
...
var handler = async (event) => {
  ...
  const { data, error, count } = await adminClient.from("access_requests").select("*", { count: "exact" }).eq("status", "PENDING").order("created_at", { ascending: true }).range(from, to);
  if (error) {
...
//# sourceMappingURL=admin-list-access-requests.js.map
