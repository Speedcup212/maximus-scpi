...
// netlify/functions/request-access.ts
var request_access_exports = {};
__export(request_access_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(request_access_exports);
...
var handler = async (event) => {
  ...
  if (!fullName || fullName.length < 2) {
    return {
      statusCode: 400,
      headers: JSON_HEADERS,
      body: JSON.stringify({ error: "Pr\xE9nom et nom requis (min 2 caract\xE8res)." })
    };
  }
  ...
  if (error) {
    const isDuplicate = error.code === "23505" || String(error.message || "").includes("duplicate") || String(error.message || "").includes("unique");
    if (isDuplicate) {
      return {
        statusCode: 200,
        headers: JSON_HEADERS,
...
//# sourceMappingURL=request-access.js.map
