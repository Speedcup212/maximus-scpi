...
// Trouver l'entrée principale LOG-IN
...
// Vérifier si l'entrée T4 2025 existe déjà
let logInT4 = data.find(s => s['Nom SCPI'] === 'Log In' && s['Période bulletin trimestriel'] === 'T4 2025');

if (!logInT4) {
  // Créer une nouvelle entrée T4 2025 basée sur l'entrée principale
  logInT4 = JSON.parse(JSON.stringify(logInMain));
  logInT4['Période bulletin trimestriel'] = 'T4 2025';
  data.push(logInT4);
  console.log('✅ Nouvelle entrée trimestrielle T4 2025 créée\n');
} else {
  console.log('✅ Entrée trimestrielle T4 2025 trouvée, mise à jour...\n');
}

// Mettre à jour les répartitions sectorielles (basées sur les données du document)
// LOG-IN étant principalement logistique et activités, on adapte les données
...
// Générer le texte de répartition sectorielle
const sectText = Object.entries(logInT4['Répartition Sectorielle JSON'])
  .map(([k, v]) => `${k} (${v}%)`)
  .join(', ');
logInT4['Répartition Sectorielle'] = sectText;

// Mettre à jour les répartitions géographiques (basées sur les données du document)
...
// Générer le texte de répartition géographique
