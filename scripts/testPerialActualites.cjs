...
if (perial['Actualités trimestrielles']) {
  const actu = perial['Actualités trimestrielles'];
  ...
  // Vérifier l'événement à venir
  const evenementAVenir = items.find(item => 
    item.toLowerCase().includes('à compter du') || 
    item.toLowerCase().includes('01/01/2026')
  );
  
  if (evenementAVenir) {
    const index = items.indexOf(evenementAVenir);
    console.log(`\n✅ Événement à venir trouvé à la position ${index + 1}`);
    console.log(`   ${evenementAVenir}`);
    
    if (index === 0) {
      console.log('\n✅ L\'événement est bien en première position !');
    } else {
      console.log(`\n⚠️  L'événement n'est pas en première position (position ${index + 1})`);
    }
  } else {
    console.log('\n❌ Événement à venir non trouvé !');
  }
