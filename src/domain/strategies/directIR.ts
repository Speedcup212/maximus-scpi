...
const computeTaxableBase = (
  income: Decimal,
  charges: Decimal,
  interest: Decimal,
  insurance: Decimal
) => income.minus(charges).minus(interest).minus(insurance);

export const runDirectIR = (ctx: DirectContext): StrategyResult => {
  const { inputs } = ctx;
  ...
  for (let year = 1; year <= ctx.years; year++) {
    ...
    const charges = ctx.fees[year - 1];
    const interest = ctx.loanInterest[year - 1];
    const insurance = ctx.loanInsurance[year - 1];

    let taxable = computeTaxableBase(income, charges, interest, insurance);
