...
const ScpiSecondaryMarketSimulator = lazy(() => import('./components/ScpiSecondaryMarketSimulator'));
...
const ExpertLogin = lazy(() => import('./components/expert/ExpertLogin'));
const ProfessionnelsPortal = lazy(() => import('./components/expert/ProfessionnelsPortal'));
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (path) {
      ...
      if (path.startsWith('pro')) {
        ...
        if (section === 'signup') {
          setCurrentView('pro-signup');
          return;
        }
        setCurrentView('pro-login');
        return;
      }
      if (path.startsWith('expert-comptable')) {
        const segments = path.split('/');
        const section = segments[1];
        if (section === 'login') {
        ...
        if (section === 'post-login') {
          setCurrentView('expert-post-login');
          return;
        }
        if (section === 'access-refused') {
          setCurrentView('expert-access-refused');
          return;
        }
        if (section === 'register') {
          setCurrentView('expert-register');
          return;
        }
        ...
        if (section === 'dossiers') {
          ...
          if (sub === '') {
            ...
            setCurrentView('expert-dossiers');
            return;
          }
          if (sub && segments[3] === 'simulations' && segments[4]) {
            setCurrentDossierId(sub);
            setCurrentSimulationId(segments[4]);
            setCurrentView('expert-dossier-simulation');
            return;
          }
          if (sub) {
            setCurrentDossierId(sub);
            setCurrentSimulationId(null);
            setCurrentView('expert-dossier-detail');
            return;
          }
          setCurrentView('expert-dossiers');
          return;
      ...
      if (path === 'professionnels') {
        setCurrentView('professionnels');
        return;
      }
      if (path.startsWith('v/')) {
        const uuid = path.replace('v/', '');
        if (uuid) {
          setVideoUuid(uuid);
          setCurrentView('anon-video');
          return;
        }
      }
      ...
      if (path === 'simulateur-marche-secondaire-scpi') {
        setCurrentView('simulateur-marche-secondaire');
        return;
      }
      ...
      if (path === 'faq') {
        ...
        window.history.replaceState({}, '', '/articles/revendre-parts-scpi-delais-marche-secondaire/');
        setCurrentArticleSlug('revendre-parts-scpi-delais-marche-secondaire');
        ...
        const allThematicPages = [
          ...
          'consultim-asset-management-scpi', 'fiducial-gerance-scpi', 'inter-gestion-reim-scpi', 'iroko-scpi', 'kyaneos-asset-management-scpi',
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (!normalizedPath || normalizedPath === '') {
        ...
        setCurrentView('pro-login');
      } else if (normalizedPath.startsWith('v/')) {
        const uuid = normalizedPath.replace('v/', '');
        if (uuid) {
          setVideoUuid(uuid);
          setCurrentView('anon-video');
        }
        ...
        setCurrentView('life-to-scpi');
      } else if (normalizedPath === 'simulateur-marche-secondaire-scpi') {
        setCurrentView('simulateur-marche-secondaire');
  ...
  const navigateToPro = (path: string) => {
    ...
    if (section === 'rapports') {
      navigateToView('pro-rapports', '/pro/rapports');
      return;
    }
    if (section === 'settings') {
    ...
    if (section === 'signup') {
      navigateToView('pro-signup', '/pro/signup');
      return;
    }
    navigateToView('pro-dashboard', '/pro/dashboard');
  };
  ...
  const handleSimulateurClick = (simulateurId: string) => {
    ...
    const simulateurMapping: Record<string, { view: string; route: string }> = {
      ...
      'revente': { view: 'simulateur-marche-secondaire', route: '/simulateur-marche-secondaire-scpi' },
  ...
  if (currentView === 'anon-video' && videoUuid) {
    return (
      <>
        <SEOHead title="Rapport SCPI | MaximusSCPI" description="Rapport trimestriel SCPI" noIndex />
        <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center text-slate-500 text-xs tracking-widest uppercase">Chargement...</div>}>
          <AnonVideoView videoUuid={videoUuid} />
        </Suspense>
      </>
    );
  }
  ...
  if (currentView === 'expert-login') {
    return (
      <div className="min-h-screen bg-slate-950 text-white">
        <SEOHead title="Connexion Expert-Comptable | MaximusSCPI" description="Connexion à l'espace Expert-Comptable MaximusSCPI" noIndex />
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><LoadingSpinner size="large" /></div>}>
          <ExpertLogin />
        </Suspense>
      </div>
  ...
  if (currentView.startsWith('expert-')) {
    return (
      <div className="min-h-screen bg-slate-950 text-white">
        ...
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><LoadingSpinner size="large" /></div>}>
          ...
          <ExpertApp
            initialSection={
              ...
              currentView === 'expert-access-refused' ? 'access-refused' :
              currentView === 'expert-register' ? 'register' :
            ...
            initialDossierId={currentDossierId || undefined}
  ...
  if (currentView === 'professionnels') {
    return (
      <div className="min-h-screen bg-slate-950 text-white">
        <SEOHead title="Espaces Professionnels | MaximusSCPI" description="Choisissez votre espace professionnel MaximusSCPI — CGP/CIF ou Expert-Comptable" noIndex />
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><LoadingSpinner size="large" /></div>}>
          <ProfessionnelsPortal />
        </Suspense>
      </div>
    );
  }
  ...
  if (currentView === 'simulateur-marche-secondaire') {
    const seo = simulatorSeoConfig['simulateur-marche-secondaire'];
    return (
      <div className={`min-h-screen bg-slate-50 dark:bg-gray-900 transition-colors duration-300 ${isDarkMode ? 'dark' : ''}`}>
        <SEOHead title={seo.title} description={seo.description} canonical={seo.canonical} schemaData={getSimulatorSchemaData('simulateur-marche-secondaire')} />
        <Header
          isDarkMode={isDarkMode}
          toggleTheme={toggleTheme}
          onContactClick={() => setIsRdvModalOpen(true)}
