...
const SelectionSidebar: React.FC<SelectionSidebarProps> = ({
  ...
  // Auto-ouverture du bloc Répartition si total ≠ 100%
  const isAllocationBalanced = useMemo(
    () => Math.abs(portfolioAnalysis.totalPercentage - 100) < 0.01,
    [portfolioAnalysis.totalPercentage]
  );
  useEffect(() => {
                    ...
                    >
                      <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                        ...
                        <span className="text-[10px] sm:text-xs text-slate-400 flex-shrink-0">
                          &mdash; {selectedScpis.length} SCPI &mdash; Total : {isAllocationBalanced ? '100 %' : `${portfolioAnalysis.totalPercentage.toFixed(1)} %`}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {isAllocationOpen && (
                          <span
                            onClick={(e) => { e.stopPropagation(); normalizePercentages(); }}
                        ...
                        <ChevronDown className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 transition-transform ${isAllocationOpen ? 'rotate-180' : ''}`} />
                      </div>
                    </button>
                    {isAllocationOpen && (<>
                    <div className="space-y-2 sm:space-y-3">
